...
for (let dataFile of dataFiles) {
    ...
    const file = require(path.join(chains_path, supplement_folder,  dataFile + '_' + supplement_folder + '.json'));
...
/*            const chainLength = chain.length;
            if (chainLength > 0) {
                ...
                // if it's from our upstream list, we should also add the actual resource
                if (dataFile === 'upstream_us_difference_lists') {
                    uniqueResources.add(unquotedResourceUrl);
                    allResources.push(unquotedResourceUrl);
                }
            } else {
                // if it's from our data, we add it (if it's from the upstream_lists, this resource is not counted)
                if (dataFile === 'upstream_us_difference_lists') {
                    uniqueResources.add(unquotedResourceUrl);
                    allResources.push(unquotedResourceUrl);
                }
